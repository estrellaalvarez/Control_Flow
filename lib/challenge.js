...
//I still need to make this not include 0^
...
//I need to come back to this one to figure out how to log the sentence instead of just numbers
...
function prompt8() {
if (someNumber <= 100) console.log(someNumber);
}
...
function prompt10() {
  const myArray = [];
 for (i = 0; i < myArray.length; i++) { if (myArray[i] <= 100) {
     push();
 }
  console.log(myArray);                 }
  return myArray;
...
function prompt11() {
  let median = (nums) => {
    ...
    return isEven ? nums[nums.length / 2 -1] : nums[Math.floor(nums.length / 2)]
  }
 console.log(median)
}
// COMMENT IN THE BELOW LINE OF CODE TO TEST YOUR OUTPUT IN THE BROWSER!
